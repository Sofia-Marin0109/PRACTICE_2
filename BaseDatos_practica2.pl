...
referencias_por_tipo_anio(Marca, Tipo, Precio, Anio, Referencias) :-
    bagof(Ref, vehiculo(Marca, Ref, Tipo, Precio, Anio), Referencias).
...
test_case1(Result) :-
    findall(Ref, (vehiculo(toyota, Ref, suv, Precio, Anio), Precio < 30000), Result).

test_case2(Tipo, Precio, Anio, Ref) :-
    referencias_por_tipo_anio(ford, Tipo, Precio, Anio, Ref).
